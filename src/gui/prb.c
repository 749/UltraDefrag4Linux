...
/**
 * @file prb.c
 * @brief Red-black trees with parent pointers.
 * @details The entire code was copied from the 
 * <a href="http://www.stanford.edu/~blp/avl/">GNU libavl</a>
 * sources. Just a few minor changes were applied to it.
...
struct prb_table *
prb_create (prb_comparison_func *compare, void *param,
...
void *
prb_find (const struct prb_table *tree, const void *item)
{
...
void *
prb_insert (struct prb_table *table, void *item)
{
...
void *
prb_replace (struct prb_table *table, void *item)
{
  void **p = prb_probe (table, item);
  if (p == NULL || *p == item)
    return NULL;
...
/* Searches for |item| in |tree|.
   ...
   and returns |NULL|. */
void *
prb_t_find (struct prb_traverser *trav, struct prb_table *tree, void *item)
{
  struct prb_node *p;
  int dir;
...
void *
prb_t_insert (struct prb_traverser *trav,
...
prb_t_next (struct prb_traverser *trav)
{
  ...
  if (trav->prb_node == NULL)
  ...
  else if (trav->prb_node->prb_link[1] == NULL)
    {
      ...
      for (p = trav->prb_node, q = p->prb_parent; ;
           p = q, q = q->prb_parent)
...
void *
prb_t_replace (struct prb_traverser *trav, void *new)
{
  void *old;
...
struct prb_table *
prb_copy (const struct prb_table *org, prb_copy_func *copy,
           prb_item_func *destroy, struct libavl_allocator *allocator)
{
...
void
prb_destroy (struct prb_table *tree, prb_item_func *destroy)
{
  struct prb_node *p, *q;
...
void
(prb_assert_insert) (struct prb_table *table, void *item)
{
  void **p = prb_probe (table, item);
...
void *
(prb_assert_delete) (struct prb_table *table, void *item)
{
